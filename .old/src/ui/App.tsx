...
export function App() {
  ...
  const {
    quickOpenOpen, setQuickOpenOpen, quickOpenQuery, setQuickOpenQuery,
    selectedIndex, results: quickOpenResults,
    closeQuickOpen, selectNext, selectPrev,
  } = useQuickOpen(files);
  const {
  ...
  return (
    ...
    >
      ...
      <QuickOpen
        ...
        onSelect={(result) => openWorkspaceFile({ name: result.name, path: result.path, kind: 'file', children: [] })}
      ...
      <StatusBar
        currentBranch={gitBranches.find((b) => b.current)?.name ?? ''}
        activeModelId={settings.selectedModelId}
        theme={settings.theme}
        hasWorkspace={!!workspacePath}
        allPermissionsOk={Object.values(settings.permissions).every(Boolean)}
        onModelClick={() => setActiveView('settings')}
        onThemeClick={() => setSettings((current) => ({ ...current, theme: current.theme === 'dark' ? 'light' : 'dark' }))}
        onPermissionsClick={() => setActiveView('settings')}
      />
    </main>
  );
}
