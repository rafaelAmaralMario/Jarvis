...
int main(int argc, char* argv[]) {
    ...
    auto* gitManager = createGitManager();
    ...
    serviceLocator.registerService("git", gitManager);
    ...
    bridge.registerHandler("getModules", [&](const QVariantList&) -> QVariant {
        ...
        for (const auto* m : loaded) {
            ...
            mod["name"] = m->manifest.name ? QString::fromUtf8(m->manifest.name) : mod["id"].toString();
    ...
    bridge.registerHandler("getModule", [&](const QVariantList& args) -> QVariant {
        ...
        for (const auto* m : loaded) {
            if (m->api && m->api->id() && id == m->api->id()) {
                QJsonObject obj;
                obj["id"] = m->api->id();
                obj["name"] = m->manifest.name ? QString::fromUtf8(m->manifest.name) : obj["id"].toString();
    ...
    bridge.registerHandler("gitStatus", [gitManager](const QVariantList& args) -> QVariant {
        if (args.isEmpty()) return QJsonArray();
        try {
            auto entries = gitManager->status(args[0].toString().toStdString());
            QJsonArray arr;
            for (const auto& e : entries) {
                QJsonObject obj;
                obj["path"] = QString::fromStdString(e.path);
                obj["status"] = QString(QChar(e.status));
                obj["staged"] = e.staged;
                obj["isUntracked"] = e.isUntracked;
                arr.append(obj);
            }
            return arr;
        } catch (...) { return QJsonArray(); }
    ...
    bridge.registerHandler("gitDiff", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return QString();
        try {
            return QString::fromStdString(gitManager->diff(args[0].toString().toStdString(), args[1].toString().toStdString()));
        } catch (...) { return QString(); }
    });

    bridge.registerHandler("gitDiffGutter", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return QJsonArray();
        try {
            auto lines = gitManager->diffGutter(args[0].toString().toStdString(), args[1].toString().toStdString());
            QJsonArray arr;
            for (const auto& l : lines) {
                QJsonObject obj;
                obj["line"] = l.line;
                obj["type"] = QString(QChar(l.type));
                arr.append(obj);
            }
            return arr;
        } catch (...) { return QJsonArray(); }
    ...
    bridge.registerHandler("gitStage", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return false;
        try {
            return gitManager->stage(args[0].toString().toStdString(), args[1].toString().toStdString());
        } catch (...) { return false; }
    });
    ...
    bridge.registerHandler("gitUnstage", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return false;
        try {
            return gitManager->unstage(args[0].toString().toStdString(), args[1].toString().toStdString());
        } catch (...) { return false; }
    });

    bridge.registerHandler("gitStageAll", [gitManager](const QVariantList& args) -> QVariant {
        if (args.isEmpty()) return false;
        try {
            return gitManager->stageAll(args[0].toString().toStdString());
        } catch (...) { return false; }
    });
    ...
    bridge.registerHandler("gitCommit", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return false;
        try {
            return gitManager->commit(args[0].toString().toStdString(), args[1].toString().toStdString());
        } catch (...) { return false; }
    });
    ...
    bridge.registerHandler("gitBranches", [gitManager](const QVariantList& args) -> QVariant {
        if (args.isEmpty()) return QJsonArray();
        try {
            auto branches = gitManager->branches(args[0].toString().toStdString());
            QJsonArray arr;
            for (const auto& b : branches) {
                QJsonObject obj;
                obj["name"] = QString::fromStdString(b.name);
                obj["isCurrent"] = b.isCurrent;
                arr.append(obj);
            }
            return arr;
        } catch (...) { return QJsonArray(); }
    ...
    bridge.registerHandler("gitCheckout", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return false;
        try {
            return gitManager->checkout(args[0].toString().toStdString(), args[1].toString().toStdString());
        } catch (...) { return false; }
    });

    bridge.registerHandler("gitCreateBranch", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return false;
        try {
            return gitManager->createBranch(args[0].toString().toStdString(), args[1].toString().toStdString());
        } catch (...) { return false; }
    });

    bridge.registerHandler("gitDeleteBranch", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 2) return false;
        try {
            return gitManager->deleteBranch(args[0].toString().toStdString(), args[1].toString().toStdString());
        } catch (...) { return false; }
    });
    ...
    bridge.registerHandler("gitPush", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 1) return false;
        try {
            std::string repo = args[0].toString().toStdString();
            std::string remote = args.size() > 1 ? args[1].toString().toStdString() : "origin";
            std::string branch = args.size() > 2 ? args[2].toString().toStdString() : "";
            return gitManager->push(repo, remote, branch);
        } catch (...) { return false; }
    });
    ...
    bridge.registerHandler("gitPull", [gitManager](const QVariantList& args) -> QVariant {
        ...
        try {
            ...
            std::string remote = args.size() > 1 ? args[1].toString().toStdString() : "origin";
            std::string branch = args.size() > 2 ? args[2].toString().toStdString() : "";
            return gitManager->pull(repo, remote, branch);
        } catch (...) { return false; }
    });
    ...
    bridge.registerHandler("gitLog", [gitManager](const QVariantList& args) -> QVariant {
        if (args.isEmpty()) return QJsonArray();
        try {
            int count = args.size() > 1 ? args[1].toInt() : 50;
            auto entries = gitManager->log(args[0].toString().toStdString(), count);
            QJsonArray arr;
            for (const auto& e : entries) {
                QJsonObject obj;
                obj["hash"] = QString::fromStdString(e.hash);
                obj["author"] = QString::fromStdString(e.author);
                obj["email"] = QString::fromStdString(e.email);
                obj["message"] = QString::fromStdString(e.message);
                obj["date"] = QString::fromStdString(e.date);
                arr.append(obj);
            }
            return arr;
        } catch (...) { return QJsonArray(); }
    ...
    bridge.registerHandler("gitIsRepo", [gitManager](const QVariantList& args) -> QVariant {
        if (args.isEmpty()) return false;
        try {
            return gitManager->isRepo(args[0].toString().toStdString());
        } catch (...) { return false; }
    });

    bridge.registerHandler("gitCurrentBranch", [gitManager](const QVariantList& args) -> QVariant {
        if (args.isEmpty()) return QString();
        try {
            return QString::fromStdString(gitManager->currentBranch(args[0].toString().toStdString()));
        } catch (...) { return QString(); }
    });

    bridge.registerHandler("gitSetCredentials", [gitManager](const QVariantList& args) -> QVariant {
        if (args.size() < 3) return false;
        try {
            gitManager->setCredentials(args[0].toString().toStdString(), args[1].toString().toStdString(), args[2].toString().toStdString());
            return true;
        } catch (...) { return false; }
    });

    qDebug() << "Registered" << 89 << "bridge handlers";
