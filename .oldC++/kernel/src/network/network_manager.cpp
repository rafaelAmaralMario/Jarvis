...
private:
    ...
    std::string getOAuthClientId(const std::string& provider) {
        ...
        if (key.empty()) {
            key = qEnvironmentVariable(QString::fromStdString(provider + "_CLIENT_ID").toUpper().toStdString().c_str()).toStdString();
        }
        return key;
    }
    ...
    std::string getOAuthClientSecret(const std::string& provider) {
        ...
        if (secret.empty()) {
            secret = qEnvironmentVariable(QString::fromStdString(provider + "_CLIENT_SECRET").toUpper().toStdString().c_str()).toStdString();
        }
        return secret;
    }
